...
port = 1883
topic = "/face_result"
topic_failed = "/face_result_failed"
topic_host = "/face_result_host"
...
def publish(client1,name, message):
	msg = f"{name}"
...
def publish_failed(client1, name):
	msg = f"{name}"
	result = client1.publish(topic_failed, msg)
	# result: [0, 1]
	status = result[0]
	if status == 0:
		print(f"Sent")
...
def publish_host(client1, name):
	msg = f"{name}"
	result = client1.publish(topic_host, msg)
	# result: [0, 1]
	status = result[0]
	if status == 0:
		print(f"Sent")
...
def alertNofi():
	api_url = f"https://z0fwjfo8tj.execute-api.ap-southeast-1.amazonaws.com/test/alertFunc"
	response = requests.get(api_url)
...
def collect_data(frame, client1):
	parser = argparse.ArgumentParser(description='Facial recognition')
	parser.add_argument('--collection', help='Collection Name', default='product')
				...
				try:
					...
					if face_matched:
						...
						if message == pre_name:
							...
							print('Hello ' + message)
							pre_name = message
							publish(client1,name, message)
					else:
						alertNofi()
						print("FAILED TO DETECT")
						publish_failed(client1, "wtf")
